...
func TestRetryFailedRemovals(t *testing.T) {
	...
	// Initialize the plugin manager with the test plugins. The uncached plugin
	// is not present in the plugin manager.
	plugins := make(map[string]*Plugin)
	for _, p := range []cleanupTestPlugin{failedPlugin, runningPlugin} {
	...
	// Add the failed plugin to the failed removals map.
	failedRemovals := make(map[string]*Plugin)
	failedRemovals[failedPlugin.String()] = &Plugin{Name: failedPlugin.name, Revision: failedPlugin.revision, InstallPath: filepath.Join(pluginDir, failedPlugin.String()), PluginType: PluginTypeDynamic}
	pm := &PluginManager{instanceID: "1234567890", plugins: plugins, failedRemovals: failedRemovals}
	...
	// Make sure all removals are removed after retry.
	if len(pm.failedRemovals) != 0 {
		t.Errorf("retryFailedRemovals(ctx): failedRemovals = %v, want empty map", pm.failedRemovals)
	}
}
