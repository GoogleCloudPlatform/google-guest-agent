...
func (o *oneShotLaunchStep) Run(ctx context.Context, p *Plugin) error {
	...
	if err != nil {
		...
		if exitErr, ok := run.AsExitError(err); ok {
			...
			errString = res.Output
		}
		p.setState(acmpb.CurrentPluginStates_EXECUTION_FAILED)
		sendEvent(ctx, p, acmpb.PluginEventMessage_PLUGIN_EXECUTION_FAILED, errString)
		results = []string{errString}
	} else {
		galog.Infof("OneShot plugin %q succeeded", p.FullName())
		p.setState(acmpb.CurrentPluginStates_EXECUTION_COMPLETED)
		sendEvent(ctx, p, acmpb.PluginEventMessage_PLUGIN_EXECUTION_COMPLETED, "")
		results = []string{res.Output}
	}
