...
func (m *PluginManager) initAdHoc(ctx context.Context, instanceID string) (*PluginManager, error) {
...
func quarantineCorruptStateFile(file string) {
	...
	if err := os.Rename(file, quarantined); err != nil {
		galog.Errorf("Failed to quarantine corrupt plugin state file %q, removing it: %v", file, err)
