...
func (m *PluginManager) GetLocalPlugin(ctx context.Context, name string) (*acpb.ConfigurePluginStates_ConfigurePlugin, error) {
	...
	if req.GetPlugin().GetRevisionId() == "" {
		// If the revision ID is not set, use the guest agent version as the revision ID.
		version := cfg.Retrieve().Core.Version
		galog.Debugf("Local plugin %q has no revision ID, using guest agent version %q", name, version)
		req.GetPlugin().SetRevisionId(version)
	}
