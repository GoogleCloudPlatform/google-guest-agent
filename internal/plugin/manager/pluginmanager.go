...
const (
	...
	manifestFile = "manifest.binarypb"
...
func (m *PluginManager) ConfigurePluginStates(ctx context.Context, req *acpb.ConfigurePluginStates, localConfig map[string]LocalPluginInstallation, localPlugin bool) {
	...
	for _, req := range toProcess {
		...
		go func(req *acpb.ConfigurePluginStates_ConfigurePlugin) {
			...
			if localConfig != nil && req.GetAction() == acpb.ConfigurePluginStates_INSTALL {
				if installation, ok := localConfig[req.GetPlugin().GetName()]; ok {
...
func (m *PluginManager) GetLocalPlugin(ctx context.Context, name string) (*acpb.ConfigurePluginStates_ConfigurePlugin, error) {
	...
	req.GetManifest().SetPluginInstallationType(acpb.PluginInstallationType_LOCAL_INSTALLATION)
...
func (m *PluginManager) StartLocalPlugins(ctx context.Context, config map[string]LocalPluginInstallation) error {
	...
	if len(configurePlugins.GetConfigurePlugins()) > 0 {
		...
		m.ConfigurePluginStates(ctx, configurePlugins, config, true)
