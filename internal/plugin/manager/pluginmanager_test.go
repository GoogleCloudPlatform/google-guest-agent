...
func TestStartLocalPlugin(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			localDir := t.TempDir()
			localPluginLinuxDir = localDir
			localPluginWindowsDir = localDir
			t.Cleanup(func() {
				localPluginLinuxDir = defaultLocalPluginLinuxDir
				localPluginWindowsDir = defaultLocalPluginWindowsDir
			})
			localPluginDir := localPluginPath()
			if err := os.Mkdir(filepath.Join(localPluginDir, "PluginA"), 0755); err != nil {
				t.Fatalf("os.Mkdir(%s) failed unexpectedly with error: %v", filepath.Join(localPluginDir, "PluginA"), err)
			}
			...
			if err := os.WriteFile(filepath.Join(localPluginDir, "PluginA", manifestFile), reqBytes, 0644); err != nil {
				t.Fatalf("os.WriteFile(%s) failed unexpectedly with error: %v", filepath.Join(localPluginDir, "PluginA", manifestFile), err)
			}
