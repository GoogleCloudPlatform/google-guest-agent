...
func (m *PluginManager) retryFailedRemovals(ctx context.Context) (bool, error) {
	m.failedRemovalsMu.Lock()
	defer m.failedRemovalsMu.Unlock()
	...
	for _, p := range pluginDirs {
		...
		if _, err := m.Fetch(p.Name()); err != nil {
			nameSplit := strings.SplitN(p.Name(), "_", 2)
			pluginName := nameSplit[0]
			pluginRevision := nameSplit[1]
			installPath := filepath.Join(pluginInstallLoc, p.Name())
			m.failedRemovals[p.Name()] = &Plugin{Name: pluginName, Revision: pluginRevision, InstallPath: installPath, PluginType: PluginTypeDynamic}
	...
	for _, link := range links {
		...
		// by the guest agent.
		if _, ok := m.failedRemovals[filepath.Base(dest)]; ok {
