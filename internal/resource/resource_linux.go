...
func (c cgroupv2Client) Apply(constraint Constraint) error {
	...
	if cpuEnabled || memoryEnabled {
		...
		if !allControllersEnabled {
			...
			if pidsEnabled {
				galog.V(2).Debugf("Enabling pids controller")
...
func (c cgroupv2Client) checkEnabledControllers(path string) (bool, bool, bool, error) {
	...
	cpuEnabled := !slices.Contains(subtreeControlContentsSplit, "-cpu") && slices.Contains(subtreeControlContentsSplit, "cpu")
	memoryEnabled := !slices.Contains(subtreeControlContentsSplit, "-memory") && slices.Contains(subtreeControlContentsSplit, "memory")
	pidsEnabled := !slices.Contains(subtreeControlContentsSplit, "-pids") && slices.Contains(subtreeControlContentsSplit, "pids")
