...
func TestEnableDisableAgent(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			w := Manager{wasEnabled: test.wasEnabled}
			...
			if w.wasEnabled != test.wantEnabled {
				t.Errorf("enableDisableAgent(ctx, true) set wasEnabled to %t, want: %t", w.wasEnabled, test.wantEnabled)
			}
		})
	}
}
