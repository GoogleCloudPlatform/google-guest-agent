...
func TestLaunchScriptRunner(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.runner.seenExecMode != run.ExecModeSync {
				t.Errorf("launchScriptRunner(ctx, %+v, %q) exec mode = %d, want %d", test.mdsClient, event, test.runner.seenExecMode, run.ExecModeSync)
			}
