...
// hanldeEvent is the entrypoint for metadata script runner. This is expected to
// invoked on receiving VM startup/shutdown/specialze(windows sysprep) event.
func hanldeEvent(ctx context.Context, mdsClient metadata.MDSClientInterface, event string) error {
...
func main() {
	...
	if err := hanldeEvent(ctx, metadata.New(), os.Args[1]); err != nil {
