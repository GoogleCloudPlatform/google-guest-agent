...
var (
	...
	credsWriter = &serialport.Logger{Port: "COM4"}
...
func (mod *module) moduleSetup(ctx context.Context, data any) error {
	...
	if err := mod.setupAccounts(ctx, desc.Instance().Attributes().WindowsKeys()); err != nil {
...
func (mod *module) eventCallback(ctx context.Context, evType string, data any, evData *events.EventData) (bool, bool, error) {
	...
	return true, false, mod.setupAccounts(ctx, desc.Instance().Attributes().WindowsKeys())
}

func (mod *module) metadataChanged(newKeys []*metadata.WindowsKey) bool {
	fmt.Println("newKeys: ", newKeys)
	fmt.Println("mod.prevKeys: ", mod.prevKeys)
...
// as needed.
func (mod *module) setupAccounts(ctx context.Context, keys []*metadata.WindowsKey) error {
	...
	err = regWriteMultiString(reg.GCEKeyBase, accountsRegKey, jsonKeys)
	if err != nil {
		return fmt.Errorf("failed to write registry keys: %w", err)
	}
...
func defaultResetPassword(ctx context.Context, key *metadata.WindowsKey) (*credentials, error) {
	...
	if key.AddToAdministrator() != nil && *key.AddToAdministrator() {
		if err = accounts.AddUserToGroup(ctx, u, accounts.AdminGroup); err != nil {
			return nil, fmt.Errorf("failed to add user to administrator group: %w", err)
		}
