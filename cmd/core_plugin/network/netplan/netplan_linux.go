...
func (sn *serviceNetplan) Setup(ctx context.Context, opts *service.Options) error {
	...
	dropinPrefix := backendDropinPrefix
	if sn.ethernetNamePrefix != "" {
		dropinPrefix = fmt.Sprintf("%s-%s", backendDropinPrefix, sn.ethernetNamePrefix)
	}
...
func (sn *serviceNetplan) writeDropin(nics []*nic.Configuration) (bool, error) {
	...
	for _, nic := range nics {
		...
		key := nic.Interface.Name()
		if sn.ethernetNamePrefix != "" {
			key = fmt.Sprintf("%s-%s", sn.ethernetNamePrefix, nic.Interface.Name())
		}
