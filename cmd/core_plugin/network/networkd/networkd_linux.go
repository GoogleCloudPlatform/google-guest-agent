...
func (nd *networkdNetdevConfig) write(sn *Module, iface string) (bool, error) {
	if file.Exists(sn.netdevFile(iface), file.TypeFile) {
		oldCfg, err := os.ReadFile(sn.netdevFile(iface))
		if err != nil {
			return false, fmt.Errorf("error reading existing networkd's .netdev config: %w", err)
		}
		newCfg, err := ini.ToString(nd)
		if err != nil {
			return false, fmt.Errorf("error marshalling networkd's .netdev config: %w", err)
		}
		if string(oldCfg) == newCfg {
			return false, nil
		}
	}
