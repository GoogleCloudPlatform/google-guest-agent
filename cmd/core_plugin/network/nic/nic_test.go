...
func TestSkipPrimaryNIC(t *testing.T) {
	tests := []struct {
		name           string
		mdsJSON        string
		expectedSize   int
		...
		{
			name:           "skip-primary-nic-multiple",
			expectedSize:   1,
			skipPrimaryNIC: true,
			mdsJSON: `
			{
		...
		{
			name:           "dont-skip-primary-nic-single",
			expectedSize:   1,
			skipPrimaryNIC: false,
			mdsJSON: `
			{
				"instance":  {
					"networkInterfaces": [
		...
		{
			name:           "dont-skip-primary-nic-multiple",
			expectedSize:   2,
			skipPrimaryNIC: false,
			mdsJSON: `
			{
		...
		t.Run(tc.name, func(t *testing.T) {
			config := &cfg.Sections{
				...
				NetworkInterfaces: &cfg.NetworkInterfaces{
					ManagePrimaryNIC: !tc.skipPrimaryNIC,
				},
			}
			...
			if err != nil {
				t.Fatalf("NewConfigs(%+v, %+v, nil) returned %v, want error? false", mds, config, err)
			}

			if len(nics) != tc.expectedSize {
				t.Errorf("NewConfigs(%+v, %+v, nil) returned %d nics, want %d", mds, config, len(nics), tc.expectedSize)
			}
		})
	}
