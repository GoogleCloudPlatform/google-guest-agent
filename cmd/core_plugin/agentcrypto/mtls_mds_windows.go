...
func (j *CredsJob) writeRootCACert(ctx context.Context, cacert []byte, outputFile string) error {
	galog.V(1).Infof("Writing root CA cert to %q", outputFile)
	...
	galog.V(1).Infof("Successfully wrote root CA cert to %q", outputFile)

	if !j.useNativeStore {
		galog.V(1).Info("Skipping system store update as it is disabled in the configuration")
		return nil
	}
