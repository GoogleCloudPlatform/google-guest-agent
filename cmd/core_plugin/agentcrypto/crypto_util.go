...
func parseCertificate(cert []byte) (*x509.Certificate, error) {
	galog.V(3).Debug("Parsing PEM certificate")
	block, _ := pem.Decode(cert)
	if block == nil {
		return nil, fmt.Errorf("failed to parse PEM certificate")
	}
	galog.V(3).Debug("Successfully parsed PEM certificate")

	galog.V(3).Debug("Parsing X509 certificate")
	x509Cert, err := x509.ParseCertificate(block.Bytes)
	if err != nil {
		return nil, fmt.Errorf("failed to parse certificate: %w", err)
	}
	galog.V(3).Debug("Successfully parsed X509 certificate")
...
func serialNumber(f string) (string, error) {
	galog.V(2).Debugf("Reading serial number from certificate %q", f)

	galog.V(3).Debugf("Reading certificate from %q", f)
	d, err := os.ReadFile(f)
	if err != nil {
		return "", fmt.Errorf("unable to read previous client credential file %q: %w", f, err)
	}
	galog.V(3).Debugf("Successfully read certificate from %q", f)

	galog.V(3).Debugf("Parsing certificate from file %q", f)
	crt, err := parseCertificate(d)
	if err != nil {
		return "", fmt.Errorf("unable to parse certificate at %q: %w", f, err)
	}
	galog.V(3).Debugf("Successfully parsed certificate from file %q", f)

	galog.V(2).Debugf("Successfully read serial number from certificate %q", f)
