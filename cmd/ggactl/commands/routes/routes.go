...
func New() *cobra.Command {
	...
	routes.AddCommand(routesRemoveCmd)
	return routes
}
...
func setupRoutes(cmd *cobra.Command, args []string) error {
	...
	// Remove all the routes before adding new ones.
...
// removeRoutes removes all routes.
func removeRoutes(cmd *cobra.Command, args []string) error {
	if len(args) > 0 {
		return fmt.Errorf("no arguments expected for remove command")
	}
	...
	opts := service.NewOptions(nil, nicConfigs)
	return rollbackRoutes(ctx, opts)
}

// rollbackRoutes removes all routes.
func rollbackRoutes(ctx context.Context, opts *service.Options) error {
	var errs []error
	for _, nicConfig := range opts.FilteredNICConfigs() {
		if err := route.RemoveRoutes(ctx, nicConfig.Interface.Name()); err != nil {
			galog.Errorf("failed to remove routes for interface %s: %v", nicConfig.Interface.Name(), err)
			errs = append(errs, err)
		}
	}
	if len(errs) > 0 {
		return fmt.Errorf("failed to remove routes: %v", errs)
	}
	return nil
}
