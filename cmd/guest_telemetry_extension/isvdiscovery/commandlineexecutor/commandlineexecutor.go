...
func ExecuteCommand(ctx context.Context, params Params) Result {
	if !exists(params.Executable) {
		slog.DebugContext(ctx, "Command executable not found", "executable", params.Executable)
	...
	restrictedArg := checkRestrictedArgs(args)
	if restrictedArg != "" {
		slog.DebugContext(ctx, "Command contains restricted argument", "executable", params.Executable,
			"args", args, "restrictedArg", restrictedArg)
		return Result{"", fmt.Sprintf("Command contains restricted argument: %q", restrictedArg), 0,
			fmt.Errorf("command contains restricted argument: %q", restrictedArg), true, false}
	}
	...
	if err != nil {
		slog.DebugContext(ctx, "Could not setup the executable environment", "executable", params.Executable, "args", args, "error", err)
	...
	if err != nil {
		...
		if len(m) > 0 {
			...
			if serr != nil {
				slog.DebugContext(ctx, "Failed to get command exit code from string match", "executable", params.Executable,
...
/*
checkRestrictedArgs returns the restricted argument found in the command args, if any.
Restricted arguments are those that can cause security issues if used in certain contexts.
*/
