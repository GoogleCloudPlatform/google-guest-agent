...
func ExecuteRules(req *defpb.DiscoveryRules, vmInfo *VMInfo) *defpb.DiscoveryResult {
	...
	for _, rule := range rules {
		foundMatch := executeRule(rule, vmInfo)
		if foundMatch {
			version := executeVersionRules(rule)
...
func evalAllCondition(all *defpb.AllCondition, vmInfo *VMInfo) bool {
	for _, condition := range all.GetConditions() {
		if !checkCondition(condition, vmInfo) {
			return false
		}
	}
	if all.HasAny() {
		if !evalAnyCondition(all.GetAny(), vmInfo) {
			return false
		}
	}
	return true
...
func evalAnyCondition(any *defpb.AnyCondition, vmInfo *VMInfo) bool {
	...
	if any.HasAll() {
		if evalAllCondition(any.GetAll(), vmInfo) {
			return true
		}
	}
	return false
...
func executeRule(rule *defpb.DiscoveryRule, vmInfo *VMInfo) bool {
...
func executeVersionRules(rule *defpb.DiscoveryRule) string {
...
func checkCondition(condition *defpb.Condition, vmInfo *VMInfo) bool {
	result := true
	...
	case defpb.Condition_StringMatch_case:
		...
		case defpb.StringMatchCondition_VmField_case:
			...
			case defpb.StringMatchCondition_VM_PROCESS_NAME:
				result = checkStringMatch(stringMatch.GetRegexMatch(), vmInfo.ProcessNames)
			case defpb.StringMatchCondition_VM_PROCESS_PATH:
				result = checkStringMatch(stringMatch.GetRegexMatch(), vmInfo.ProcessPaths)
			case defpb.StringMatchCondition_VM_OS_NAME:
				result = checkStringMatch(stringMatch.GetRegexMatch(), []string{vmInfo.OSName})
			case defpb.StringMatchCondition_VM_CLI_ARGS:
				result = checkStringMatch(stringMatch.GetRegexMatch(), vmInfo.ProcessArgs)
			case defpb.StringMatchCondition_VM_ENV_VARS:
				result = checkStringMatch(stringMatch.GetRegexMatch(), vmInfo.ProcessEnvVars)
			default:
				// This should never happen. Return false if it does.
				return false
			}
...
func checkStringMatch(pattern string, values []string) bool {
